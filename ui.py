...
class ChatbotInterface(Frame):
    ...
    # File
       ...
       # Options
        ...
        help_option.add_command(label="About AlchemyChatBot", command=self.msg)
...
#         self.last_sent_label(date=" ")
    ...
    def msg(self):
        tkinter.messagebox.showinfo("AlchemyChatBot v1.0","AlchemyChatBot is a chatbot for answering common data science questions.")
    def about(self):
        tkinter.messagebox.showinfo("AlchemyChatBot Developer","1. Girish V (GirishCodeAlchemy) \n")

    def send_message_insert(self, message):
        user_input = self.entry_field.get()
        pr1 = "You : " + user_input + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, pr1, "user_question")
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)

        ob=get_chatbot_response(user_input)
        pr="AlchemyBot : " + ob + "\n"
        self.text_box.configure(state=NORMAL)
        # self.text_box.insert(END, pr)
        self.text_box.insert(END, pr, "answer")
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)
#         self.last_sent_label(str(time.strftime( "Last message sent: " + '%B %d, %Y' + ' at ' + '%I:%M %p')))
        self.entry_field.delete(0,END)
        self.text_box.tag_config("user_question", foreground="red")
        self.text_box.tag_config("answer", foreground="green", font="bold")
        time.sleep(0)
...
p1 = PhotoImage(file = 'alchemychatbot.png')
